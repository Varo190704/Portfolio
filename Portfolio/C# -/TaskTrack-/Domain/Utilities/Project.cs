...
public class Project
{
    ...
    public Project(string name, string description, DateOnly startDate,
        DateOnly endDate, int duration, string adminEma, string liderEma)
    {
        Name = name;
        Description = description;
        Admin = adminEma;
    ...
    public void assTask(Task task)
    {
        ...
        pTask.Resources = new List<Resource>();
        Tasks.Add(pTask);
        refreshTimes();
    ...
    public void assLider(User user)
    {
        if (!Equals(user, null))
        {
            Lider = user.Email;
            Members.Add(user);
            user.projects.Add(this);
            user.Types.Add(UserType.ProjectLider);
        }
        else
        {
            throw new ArgumentException("Invalid Admin");
    ...
    {
        Task tN = Tasks.Find(t => t.Name == task.Name);
        if (tN != null)
        {
            tN.Description = task.Description;
            tN.Duration = task.Duration;
            tN.Status = task.Status;
            tN.EndDate = tN.StartDate.AddDays(task.Duration);
            tN.LateEndDate = tN.EndDate.AddDays(tN.Slak);
        }
    }
